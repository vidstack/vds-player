...
export interface MediaProviderBridge extends MediaProviderBridgedProperties, MediaProviderBridgedMethods {
    /**
    ...
    readonly mediaProviderConnectedQueue: RequestQueue<string | symbol, () => void | Promise<void>>;
    readonly mediaProviderDisconnectDisposal: DisposalBin;
}
export declare type MediaProviderBridgedProperties = Pick<VideoElement, 'autoPiP' | 'autoplay' | 'buffered' | 'canPlay' | 'canPlayThrough' | 'canRequestFullscreen' | 'controls' | 'controlsList' | 'crossOrigin' | 'currentPoster' | 'currentSrc' | 'currentTime' | 'defaultMuted' | 'defaultPlaybackRate' | 'disablePiP' | 'disableRemotePlayback' | 'duration' | 'ended' | 'error' | 'fullscreen' | 'fullscreenOrientation' | 'height' | 'live' | 'loop' | 'mediaType' | 'muted' | 'networkState' | 'paused' | 'played' | 'playing' | 'playsinline' | 'poster' | 'preload' | 'readyState' | 'seekable' | 'seeking' | 'src' | 'srcObject' | 'started' | 'viewType' | 'volume' | 'waiting' | 'width'>;
